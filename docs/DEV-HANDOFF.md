...
**▶ PO DIRECTIVES 2026-08-06 — spec: `docs/handoff-argpreserve-and-translations-2026-08-06.md`.** ① **Never rewrite a call's argument** — PO: *"I don't want `isAlive(contact.contact)`, wouldn't make sense."* Record the real arg text on the 4 well-known kinds and emit it verbatim; if args don't match, **don't classify** (keep `raw`). Fix together with the statement-loss gate — same root error (partial recognition that discards instead of declining). Add `server/templates/malaria/tasks.js` + a cht-default helper body as byte-stability fixtures. ② **Add-a-translation-key, promoted out of the P2 tail** — this settles the open scope question: **no literal-title shortcut**, real keys stay the path. `TranslationsEditor` gets "+ Add key" as a grouped dropdown (① referenced-but-missing = dangling translation refs the tool can already compute · ② convention templates instantiated with real project names, `task.<name>.title` / `targets.<id>.title` / `contact.type.<id>` · ③ CHT core keys · ④ custom, validated) + `TasksEditor`'s title becomes a key **picker showing resolved strings** plus "＋ New translation key…" with inline EN/NE entry. Adds to **every** locale file. Unblocks the durable geriatric regression spec (17 task rows are FRICTION on the title axis alone).
