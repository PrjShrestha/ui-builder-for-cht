...
5. **Lint + `.gitattributes`** — lint RED (~90: mostly the `eslint.config.js` browser-globals gap + 17 real `no-useless-escape`); add `.gitattributes` (`*.json eol=lf`) so the 3 FHIR CRLF tests pass on a fresh clone; then re-add `pnpm lint` to CI.
6. **`client/vite.config.run.mjs`** — throwaway local run config, intentionally uncommitted; delete / fold into `vite.config.ts` / `.gitignore` (P2).
