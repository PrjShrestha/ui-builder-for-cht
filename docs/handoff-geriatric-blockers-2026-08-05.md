...
**Design:** mirror the in-form builder's connector approach at minimal scope: an **and/or connector pill between rule rows**, where consecutive OR-joined rows form one group, serialized to exactly the guard shape `guardGroups` already parses (so parse→serialize→parse is stable by construction). No mixed-precedence UI needed — `(A || B) && C` as "OR-group then AND" covers the spec; anything beyond falls back to raw JS as today.
